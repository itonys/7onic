...
var require_map_generator = __commonJS({
  "node_modules/postcss/lib/map-generator.js"(exports2, module2) {
    ...
    var MapGenerator = class {
      ...
      toFileUrl(path7) {
        let cached = this.memoizedFileURLs.get(path7);
        if (cached) return cached;
        if (pathToFileURL) {
      ...
      toUrl(path7) {
        let cached = this.memoizedURLs.get(path7);
        if (cached) return cached;
        if (sep === "\\") {
          path7 = path7.replace(/\\/g, "/");
        }
        let url = encodeURI(path7).replace(/[#?]/g, encodeURIComponent);
        this.memoizedURLs.set(path7, url);
        return url;
      }
    };
              ...
              if (!NOT_VISITORS[event]) {
                if (typeof plugin2[event] === "object") {
                  for (let filter in plugin2[event]) {
                    if (filter === "*") {
                      add2(plugin2, event, plugin2[event][filter]);
                    } else {
                      add2(
                        plugin2,
...
function pathExists(cwd, relativePath) {
  return import_node_fs2.default.existsSync(import_node_path2.default.join(cwd, relativePath));
}
...
function readPackageDeps(cwd) {
  try {
    const pkg = JSON.parse(import_node_fs2.default.readFileSync(import_node_path2.default.join(cwd, "package.json"), "utf-8"));
...
function formatMessage({ message, hint, link }) {
  const parts = [message];
  if (hint) parts.push(hint);
  if (link) parts.push(`${LINK_PREFIX}${link}`);
  return parts.join(SECTION_SEPARATOR);
}
function gracefulExit(options) {
  pt(formatMessage(options));
  process.exit(1);
}
function userCancel(message = DEFAULT_CANCEL_MESSAGE) {
  pt(message);
  process.exit(0);
}
...
function buildCssImports({ version, includeTailwind }) {
...
function requiredTokenImports(version) {
...
function isStandaloneStar(rule2) {
  return rule2.selectors.length === 1 && rule2.selectors[0].trim() === "*";
}
function isTopLevel(rule2) {
  return rule2.parent?.type === "root";
}
function wrapStarReset(input) {
  const root2 = postcss_default.parse(input);
  const targets = [];
  root2.walkRules((rule2) => {
    if (isStandaloneStar(rule2) && isTopLevel(rule2)) {
      targets.push(rule2);
    }
  });
  for (const rule2 of targets) {
    const layer = postcss_default.atRule({ name: "layer", params: "base" });
    rule2.replaceWith(layer);
    layer.append(rule2);
  }
  return {
    wrappedCount: targets.length,
    output: targets.length === 0 ? input : root2.toString()
  };
}
...
function parseJsonc(source) {
  const errors = [];
  const data = parse3(source, errors, PARSE_OPTIONS);
  if (errors.length > 0) {
...
var DEFAULT_CSS_PATH = "src/app/globals.css";
var DEFAULT_COMPONENTS_ALIAS = "@/components/ui";
var DEFAULT_UTILS_ALIAS = "@/lib/utils";
var INSTALL_PAGE = "https://7onic.design/components/installation";
var NEXT_TS_DOCS = "https://nextjs.org/docs/app/api-reference/config/typescript";
var VITE_TS_DOCS = "https://vitejs.dev/guide/features.html#typescript";
...
async function init(args) {
  ...
  if (configExists(projectRoot) && !flagYes) {
    const overwrite = await ot2({ message: "7onic.json already exists. Overwrite?" });
    if (q(overwrite) || !overwrite) userCancel();
  }
  ...
  const { tailwindVersion, cssPath, componentsAlias, utilsAlias } = await resolveConfig(projectRoot, { flagYes, forcedTailwindVersion, tailwind });
  ...
  const v3ConfigPath = tailwindVersion === 3 && tailwind.version === 3 ? tailwind.configPath : void 0;
  const finalConfig = {
    $schema: "https://7onic.design/schema/7onic.json",
    tailwind: {
      version: tailwindVersion,
      ...v3ConfigPath !== void 0 ? { config: v3ConfigPath } : {},
      css: cssPath
    },
    aliases: {
...
async function resolveConfig(projectRoot, { flagYes, forcedTailwindVersion, tailwind }) {
  const cssDefault = findFirstExisting(projectRoot, CSS_CANDIDATES) ?? DEFAULT_CSS_PATH;
  if (flagYes) {
    const tailwindVersion = forcedTailwindVersion ?? tailwind.version;
    O2.info(`Using defaults: Tailwind v${tailwindVersion}, CSS: ${cssDefault}`);
    return {
  ...
  return await dt(
    {
      componentsAlias: () => Ot({ message: "Components path alias", placeholder: DEFAULT_COMPONENTS_ALIAS, defaultValue: DEFAULT_COMPONENTS_ALIAS }),
      utilsAlias: () => Ot({ message: "Utils path alias", placeholder: DEFAULT_UTILS_ALIAS, defaultValue: DEFAULT_UTILS_ALIAS }),
      tailwindVersion: () => _t({
      ...
      cssPath: () => Ot({ message: "CSS file path", placeholder: cssDefault, defaultValue: cssDefault })
    },
    { onCancel: () => userCancel() }
  );
}
var BASE_DEPS = ["@7onic-ui/tokens", "clsx", "tailwind-merge", "class-variance-authority"];
async function installBaseDeps(projectRoot, pm, needsTypesNode) {
  const s = fe();
...
function injectCssImports(projectRoot, cssPath, version) {
  const cssFullPath = import_node_path5.default.join(projectRoot, cssPath);
  if (!import_node_fs5.default.existsSync(cssFullPath)) {
    import_node_fs5.default.mkdirSync(import_node_path5.default.dirname(cssFullPath), { recursive: true });
    import_node_fs5.default.writeFileSync(cssFullPath, buildCssImports({ version, includeTailwind: true }), "utf-8");
    O2.success(`Created ${cssPath} with token imports`);
    return;
  }
  const content = import_node_fs5.default.readFileSync(cssFullPath, "utf-8");
  const required = requiredTokenImports(version);
  const missing = required.filter((line) => !content.includes(line));
  if (missing.length === 0) {
    O2.info(`Token imports already present in ${cssPath}`);
    return;
  }
  const updated = mergeCssImports(content, missing, version);
  import_node_fs5.default.writeFileSync(cssFullPath, updated, "utf-8");
  O2.success(`Added token imports to ${cssPath}`);
}
function mergeCssImports(content, missingLines, version) {
...
function cleanupViteStub(projectRoot) {
  const scanResults = scanViteTemplate(projectRoot);
  if (scanResults.length === 0) return;
  for (const result of scanResults) {
    applyCleanup(result);
  }
  const summary = scanResults.map((r) => `  ${r.file} (${pluralize(r.matches.length, "block")})`).join("\n");
...
function wrapStarResetIfPresent(projectRoot, cssPath) {
  const fullPath = import_node_path5.default.join(projectRoot, cssPath);
  if (!import_node_fs5.default.existsSync(fullPath)) return;
  const original = import_node_fs5.default.readFileSync(fullPath, "utf-8");
  const { wrappedCount, output } = wrapStarReset(original);
  if (wrappedCount === 0) return;
  import_node_fs5.default.writeFileSync(fullPath + ".bak", original, "utf-8");
  import_node_fs5.default.writeFileSync(fullPath, output, "utf-8");
  O2.success(
...
function showV3PresetNote(projectRoot, configPath) {
  const configFullPath = import_node_path5.default.join(projectRoot, configPath);
  if (!import_node_fs5.default.existsSync(configFullPath)) return;
  const content = import_node_fs5.default.readFileSync(configFullPath, "utf-8");
...
` + import_picocolors.default.dim(`  module.exports = {
`) + import_picocolors.default.green(`    presets: [require('@7onic-ui/tokens/tailwind/v3-preset')],
`) + import_picocolors.default.dim(`    // ... rest of config
`) + import_picocolors.default.dim(`  }`),
    "Tailwind v3 Setup"
  );
}
function ensureCnUtility(projectRoot, utilsAlias) {
  ...
  const relPath = import_node_path5.default.relative(projectRoot, utilsFullPath);
  if (!import_node_fs5.default.existsSync(utilsFullPath)) {
    import_node_fs5.default.mkdirSync(import_node_path5.default.dirname(utilsFullPath), { recursive: true });
    import_node_fs5.default.writeFileSync(utilsFullPath, CN_UTIL, "utf-8");
    O2.success(`Created ${relPath}`);
    return;
  }
  const content = import_node_fs5.default.readFileSync(utilsFullPath, "utf-8");
...
async function ensureViteAlias(projectRoot, framework) {
  const tsconfigName = framework.hasTsconfigApp ? "tsconfig.app.json" : "tsconfig.json";
  const tsconfigPath = import_node_path5.default.join(projectRoot, tsconfigName);
  const viteConfigPath = import_node_path5.default.join(projectRoot, framework.configPath);
  ...
  if (tsconfigHasAlias && viteHasAlias) {
    O2.info("@/ path alias already configured (tsconfig + vite.config)");
    return false;
  }
  ...
  O2.success(`Configured @/ path alias in ${tsconfigName} and ${framework.configPath}`);
  return viteWasPatched;
}
function readTsconfigAlias(tsconfigPath) {
  if (!import_node_fs5.default.existsSync(tsconfigPath)) return false;
  try {
    const raw = import_node_fs5.default.readFileSync(tsconfigPath, "utf-8");
    const tc = parseJsonc(raw);
    return Boolean(tc.compilerOptions?.paths?.["@/*"]);
  } catch {
    return false;
  }
}
function readViteAlias(viteConfigPath) {
  if (!import_node_fs5.default.existsSync(viteConfigPath)) return false;
  const content = import_node_fs5.default.readFileSync(viteConfigPath, "utf-8");
  return /['"]@['"]\s*:/.test(content);
}
function patchTsconfigAlias(tsconfigPath) {
  ...
  try {
    ...
    import_node_fs5.default.writeFileSync(tsconfigPath, JSON.stringify(updated, null, 2) + "\n", "utf-8");
    return { ok: true };
  } catch (err) {
    return { ok: false, error: err instanceof Error ? err.message : String(err) };
  }
}
function patchViteConfigAlias(viteConfigPath) {
  if (!import_node_fs5.default.existsSync(viteConfigPath)) {
    return { ok: false, error: "file not found" };
  }
  try {
...
var logger = {
  info(msg) {
    console.log(import_picocolors2.default.cyan("i"), msg);
  },
  success(msg) {
  ...
  title(msg) {
    console.log(import_picocolors2.default.bold(msg));
  }
};
