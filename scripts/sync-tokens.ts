...
function writeTextFile(filePath: string, content: string): WriteStatus {
...
async function main(): Promise<void> {
  ...
  const statuses: Array<{ label: string; status: WriteStatus }> = [
    { label: 'tokens/css/variables.css',      status: writeTextFile(VARIABLES_CSS_PATH, variablesCss) },
    { label: 'tokens/css/themes/light.css',   status: writeTextFile(THEME_LIGHT_PATH, themeLight) },
    { label: 'tokens/css/themes/dark.css',    status: writeTextFile(THEME_DARK_PATH, themeDark) },
    { label: 'tokens/tailwind/v3-preset.js',  status: writeTextFile(V3_PRESET_PATH, v3Preset) },
    { label: 'tokens/tailwind/v4-theme.css',  status: writeTextFile(V4_THEME_PATH, v4Theme) },
    { label: 'tokens/js/index.js',            status: writeTextFile(JS_CJS_PATH, jsTokens.cjs) },
    ...
    { label: 'src/styles/docs-site.css',      status: writeTextFile(DOCS_SITE_CSS_PATH, docsSiteCss) },
  ]

  // Write deprecated aliases if any renames detected
  const DEPRECATED_CSS_PATH = path.join(ROOT, 'tokens/css/deprecated.css')
  if (deprecatedCss) {
    const depStatus = writeTextFile(DEPRECATED_CSS_PATH, deprecatedCss)
    statuses.push({ label: 'tokens/css/deprecated.css (backwards compat)', status: depStatus })
  }
  ...
  for (const { label, status } of statuses) {
    console.log(`   📄 ${label} ${formatStatusBadge(status)}`)
  }
}
